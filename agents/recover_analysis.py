...
class RecoveryAnalyzer:
    ...
    def __init__(self, data_dir: Path, force: bool = False):
        ...
        # Load historian manager once — reused across all triads
        logger.info("Loading historian personas and embeddings...")
        self.hist_manager = HistorianManager(n_historians=25)
        self.hist_manager.create_historian_personas()
        self.hist_manager.compute_historian_embeddings()
        logger.info(f"Loaded {len(self.hist_manager.personas)} historian personas")
    ...
    def recover_triad_data(self, triad_id: int) -> Tuple[List[HistorianPersona], List[Dict], Dict, List[Dict]]:
        ...
        # Match historian IDs against pre-loaded personas in self.hist_manager
        historian_ids = [triad['historian_1_id'], triad['historian_2_id'], triad['historian_3_id']]
        historians = []
        for hist_id in historian_ids:
            matching = [p for p in self.hist_manager.personas if p.historian_id == hist_id]
            if matching:
